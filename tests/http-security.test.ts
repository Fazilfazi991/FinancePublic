...
describe.skipIf(!base)('unauthenticated finance API',()=>{
 for(const path of ['/api/accounts','/api/debts','/api/transactions','/api/settings'])it(`rejects GET ${path}`,async()=>expect((await fetch(`${base}${path}`,{cache:'no-store'})).status).toBe(401));
