...
export function TransactionRow({ transaction, variant = "activity", accountId }: { transaction: Transaction; variant?: "activity" | "account"; accountId?: string }) {
  ...
  const account = accounts.find(item => item.id === transaction.accountId);
  const Icon = variant === "account" ? Wallet : transaction.type === "income" ? ArrowDownLeft : transaction.type === "transfer" ? ArrowLeftRight : ArrowUpRight;
  const positive = transaction.type === "income" || (transaction.type === "transfer" && transaction.toAccountId === accountId);
  return <TransactionActions transaction={transaction}>
    <button type="button" aria-label={`Open actions for ${transaction.description || transaction.category}`} className={cn("flex w-full items-center gap-3 px-4 text-left transition-colors hover:bg-secondary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary",variant === "activity" ? "min-h-[76px]" : "min-h-[68px]")}>
      <span className={cn("grid shrink-0 place-items-center rounded-xl",variant === "activity" ? "h-11 w-11" : "h-10 w-10",variant === "activity" && (transaction.type === "income" ? "bg-emerald-500/10 text-emerald-600" : transaction.type === "transfer" ? "bg-sky-500/10 text-sky-500" : "bg-secondary"),variant === "account" && "bg-secondary")}><Icon className="h-5 w-5"/></span>
      ...
      <span className={cn("tabular whitespace-nowrap font-semibold",variant === "activity" ? "text-sm" : "",variant === "activity" && (transaction.type === "income" ? "text-emerald-600" : transaction.type === "transfer" ? "text-sky-500" : "text-foreground"))}>{variant === "account" ? (positive ? "+" : "-") : transaction.type === "income" ? "+" : transaction.type === "expense" ? "−" : ""}{formatCurrency(transaction.amount,transaction.currency || settings.currency)}</span>
    </button>
  </TransactionActions>;
}
