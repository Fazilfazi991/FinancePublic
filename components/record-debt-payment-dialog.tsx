...
export function RecordDebtPaymentDialog({children,initialDebtId="",open:controlledOpen,onOpenChange}:{children?:React.ReactNode;initialDebtId?:string;open?:boolean;onOpenChange?:(open:boolean)=>void}){
 ...
 const submit=async(event:React.FormEvent)=>{event.preventDefault();const value=Number(amount);if(!debtId||!accountId||!value||pending)return;setPending(true);setError("");try{await recordDebtPayment(debtId,accountId,value,date,notes);trackEventSafely('debt_payment_added',{source:'dialog'});setAmount("");setNotes("");setOpen(false)}catch(e){setError(e instanceof Error?e.message:'The payment could not be recorded.')}finally{setPending(false)}};
 return <Dialog open={open} onOpenChange={setOpen}>{(children||controlledOpen===undefined)&&<DialogTrigger asChild>{children||<Button><CreditCard className="mr-2 h-4 w-4"/>Record payment</Button>}</DialogTrigger>}<DialogContent className="finance-sheet max-h-[92dvh] overflow-y-auto border-border bg-card sm:max-w-[460px] sm:rounded-2xl max-sm:top-auto max-sm:bottom-0 max-sm:translate-y-0 max-sm:rounded-b-none max-sm:rounded-t-3xl [&_input]:text-base"><DialogHeader><DialogTitle className="text-xl">Record debt payment</DialogTitle></DialogHeader><form onSubmit={submit} className="space-y-4 py-4">
  ...
  {error&&<p role="alert" className="text-sm text-destructive">{error}</p>}<DialogFooter><Button type="submit" disabled={!debts.length||!accounts.length||pending} className="min-h-12 w-full rounded-xl font-semibold">{pending?'Recording…':'Record payment'}</Button></DialogFooter>
