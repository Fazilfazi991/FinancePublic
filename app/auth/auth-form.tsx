...
import {trackEvent,trackEventSafely} from '@/lib/analytics/client';
...
export function AuthForm({next='/'}:{next?:string}){
  ...
  const sendCode=async(target:string)=>{
    trackEventSafely('signup_started',{method:'email'},'web');
  ...
  const verify=async(e:React.FormEvent)=>{e.preventDefault();if(!/^\d{6}$/.test(code)){setMessage('Enter the complete 6-digit code.');return}setPending(true);setMessage('');const{error}=await supabase.auth.verifyOtp({email:submittedEmail,token:code,type:'email'});if(error){setMessage(friendlyOtpError(error.message));setPending(false);return}await trackEvent('login_completed',{method:'email'},'web');router.replace(next);router.refresh()};
  const google=async()=>{setPending(true);setMessage('');trackEventSafely('signup_started',{method:'google'},'web');const{error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:`${location.origin}/auth/callback?next=${encodeURIComponent(next)}`}});if(error){setMessage('Google sign-in is not configured or is temporarily unavailable.');setPending(false)}};
