...
export function AuthForm({next='/'}:{next?:string}){
  const[email,setEmail]=useState(''),[submittedEmail,setSubmittedEmail]=useState(''),[code,setCode]=useState(''),[message,setMessage]=useState(''),[pending,setPending]=useState(false),[cooldown,setCooldown]=useState(0);
  ...
  const sendCode=async(target:string)=>{
    ...
    else if(otpEnabled){setSubmittedEmail(target);setCooldown(RESEND_SECONDS)}else setMessage('Check your email for the secure sign-in link.');
    setPending(false);
  };
  ...
  return <div className="space-y-5"><Button className="h-12 w-full" onClick={google} disabled={pending}>Continue with Google</Button><div className="flex items-center gap-3 text-xs text-muted-foreground"><span className="h-px flex-1 bg-border"/>or<span className="h-px flex-1 bg-border"/></div><form onSubmit={request} className="space-y-3"><label htmlFor="email" className="text-sm font-medium">Email address</label><Input id="email" type="email" value={email} onChange={event=>setEmail(event.target.value)} placeholder="user@example.com" autoComplete="email" required className="h-12"/><Button variant="outline" className="h-12 w-full" disabled={pending}>{pending?(otpEnabled?'Sending code…':'Sending link…'):(otpEnabled?'Send verification code':'Email me a secure sign-in link')}</Button></form>{message&&<p role="alert" className="rounded-xl bg-destructive/10 p-3 text-sm text-destructive">{message}</p>}</div>;
}
