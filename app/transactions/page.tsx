...
export default function TransactionsPage() {
  ...
  const filtered = useMemo(() => transactions.filter(t => { const d=new Date(t.date); return d.getMonth()===month&&d.getFullYear()===year; }).filter(t => filter==="all"||t.type===filter).filter(t => `${t.description} ${t.category} ${accounts.find(a=>a.id===t.accountId)?.name||""}`.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>new Date(b.createdAt||b.date).getTime()-new Date(a.createdAt||a.date).getTime()), [transactions,accounts,filter,search,month,year]);
  const groups=filtered.reduce<Record<string,Transaction[]>>((all,t)=>{(all[t.date]||=[]).push(t);return all},{});

  return <main className="mobile-page">
    <header className="flex items-center justify-between gap-3"><div><h1 className="text-3xl font-bold tracking-tight">Activity</h1><p className="text-sm text-muted-foreground">{filtered.length} transactions</p></div><div className="flex gap-1"><button aria-label="Search activity" onClick={()=>setSearchOpen(v=>!v)} className="tap-target grid place-items-center rounded-full bg-secondary"><Search className="h-5 w-5"/></button><button aria-label="Transaction filters" className="tap-target grid place-items-center rounded-full bg-secondary"><SlidersHorizontal className="h-5 w-5"/></button></div></header>
...
function dateLabel(value:string){const date=new Date(`${value}T12:00:00`),today=new Date(),yesterday=new Date();yesterday.setDate(today.getDate()-1);if(date.toDateString()===today.toDateString())return"Today";if(date.toDateString()===yesterday.toDateString())return"Yesterday";return date.toLocaleDateString(undefined,{weekday:"long",month:"short",day:"numeric"})}
