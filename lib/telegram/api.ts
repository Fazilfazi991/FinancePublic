...
async function call(method:string,body:Record<string,unknown>){const response=await fetch(endpoint(method),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});if(!response.ok)throw new Error('Telegram request failed')}
...
export const financeKeyboard=()=>({inline_keyboard:[[{text:'Open Finance',url:'https://finance-public-sigma.vercel.app/accounts'}]]});
